...
describe('GenerateUniqueSlugUseCase', () => {
	...
	it('Should generate an unique slug with minimal length', async () => {
		...
		const slug = await generator.generateUniqueSlug(
			TargetUrl.create('https://google.com'),
		)
	...
	it('Should throw an error if maxAttempts is exceeded for all lengths', async () => {
		...
		//2. ACT: call the generator
		await expect(
			generator.generateUniqueSlug(TargetUrl.create('https://google.com')),
		).rejects.toThrow(SlugGenerationExhaustedError)
	})
})
